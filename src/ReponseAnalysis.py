...
class ProductsAnalysisAgent:
    ...
    def getProductSaleAmount(self, productSoup):
        ...
        if historyTag == None or historyTag.span == None:
...
class DomesticTravelAnalysisAgent:
    ...
    def getPopularCityUrls(self):
        ...
        allCitiesTag = soup.find("dt", text = "热门省份旅游")

        if allCitiesTag == None:
            return None

        allCitiesTagNextSibling = allCitiesTag.find_next_sibling()
        allCityUrls = []
        for child in allCitiesTagNextSibling.children:
            if child.string == '\n':
                continue
            else:
...
class Starter:
    def startDomesticTravelAnalysis(self, url):
        ...
        for cityUrl in allCityUrls:
            ...
            for page in range(min(pageCount, NUMBER_OF_PAGES_FOR_CITY)):
